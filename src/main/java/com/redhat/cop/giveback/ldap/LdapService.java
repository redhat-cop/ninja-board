package com.redhat.cop.giveback.ldap;
...
public class LdapService {
  ...
  public LdapService(){
    ldapProvider=Config.get().getOptions().get("users.ldap.provider");
    if (null==ldapProvider) ldapProvider=System.getenv("USERS_LDAP_PROVIDER");
    Preconditions.checkArgument(ldapProvider!=null, "users.ldap.provider option or USERS_LDAP_PROVIDER environment variable is mandatory");
  }
...
//      String searchDN=String.format("(&(objectclass=Person)(%s=%s))",field, value);
      ...
      List<User> result2=new ArrayList<LdapService.User>();
