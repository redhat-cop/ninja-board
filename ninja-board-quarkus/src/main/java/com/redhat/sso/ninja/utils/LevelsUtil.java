...
public class LevelsUtil {
    ...
    public static LevelsUtil get() {
        if (null == instance)
            instance = new LevelsUtil(Config.get().getOptions().get("thresholds"));
        return instance;
    }
    ...
    public static void main(String[] asd) {
        ...
        for (int i = 0; i <= 27; i++) {
            System.out.println("getLevel(" + i + ") - = " + l.getLevelGivenPoints(i));// +", nextLevel will be
                                                                                      // "+l.getNextLevel(i));
        }
    ...
    public Tuple<Integer, String> getLevel(String levelName) {
        for (Tuple<Integer, String> l : levels) {
            if (l.getRight().equals(levelName))
                return l;
        }
        return null;
    }
    ...
    public Tuple<Integer, String> getLastLevel(String currentLevelName) {
        if (levels.get(0).getRight().equals(currentLevelName))
            return levels.get(0);
    ...
    // logic: last one you hit where the points are greater than required for the
    // level
